...
bool tryCastVariant(std::variant<V1...> value,
                    Option<std::variant<V2...>> &result) {
  return std::visit(
      overloads{[&](auto &&inner) -> bool {
...
TSRange nodeRange(TSNode node) {
  return {ts_node_start_point(node), ts_node_end_point(node),
          ts_node_start_byte(node), ts_node_end_byte(node)};
}

std::string_view nodeName(TSNode node) {
  return ts_language_symbol_name(tree_sitter_branescript(),
                                 ts_node_symbol(node));
}
...
template <ts_symbol_identifiers... Types> bool nodeIsSymbol(TSNode node) {
...
struct ScopedScope {
  ...
  ScopedScope(std::list<Node<TextContext>> &scopes, Node<TextContext> node)
      : scopes(scopes), node(node) {}
...
struct ParserAPI {
  ...
  void verboseMessage(TSNode ctx, std::string message) {
    messages.emplace_back(MessageType::Verbose, nodeRange(ctx),
                          std::move(message));
  }
  ...
  void warningMessage(TSNode ctx, std::string message) {
    messages.emplace_back(MessageType::Warning, nodeRange(ctx),
                          std::move(message));
  }

  void errorMessage(TSNode ctx, std::string message) {
    messages.emplace_back(MessageType::Error, nodeRange(ctx),
                          std::move(message));
  }
  ...
  bool expectField(TSNode node,
                   const std::vector<ts_symbol_identifiers> &types) {
    auto nodeType = ts_node_symbol(node);
    if (nodeType) {
      for (auto t : types) {
        if (t == nodeType)
  ...
  template <typename T> Node<T> makeNode(TSNode context) {
  ...
  Option<TextContextNode> parse(TSNode node) {
    ...
    default:
      ...
      std::cout
          << std::format(
                 "Parser not implemented for symbol {} could not parse \"{}\"",
                 nodeName(node), nodeText(node))
          << std::endl;
      return None();
    }
  }
  ...
  Option<ExpressionContextNode> parseExpression(TSNode node) {
    ...
    if (!out)
      errorMessage(node, std::format("Expected expression, but found: {}",
                                     nodeText(node)));
    return out;
  }
  ...
  Option<Node<ConstValueContext>> parseNumber(TSNode root) {
    ...
    float value;
    /*auto [ptr, res] = std::from_chars(text.data(), text.data() + text.size(),
     * value);*/
    /*if(res == std::errc())*/
    /*{*/
    /*    errorMessage(root, std::format("Error parsing float: {}",
     * std::make_error_code(res).message()));*/
    /*    return None();*/
    /*}*/
  ...
  Option<Node<ScopeContext>> parseBlock(TSNode root) {
    ...
    if (expressions) {
      foreachNamedNodeChild(root, [&](TSNode node) {
        logMessage(node,
                   std::format("parsing block child: {}", nodeText(node)));
        auto expr = parseExpression(node);
        if (!expr) {
          errorMessage(node,
                       std::format("Was expecting expression but found: {}",
                                   nodeText(node)));
          return;
        }
  ...
  Option<Node<PipelineContext>> parsePipeline(TSNode root) {
    ...
    advanceWhileSymbol<sym_pipelineStage>(
        tsStagesNode.value(), [&](TSNode tsStageNode) {
  ...
  Option<Node<ModuleContext>> parseModule(TSNode root) {
    ...
    if (!firstDef)
      return Some(mod);
    for (auto currentDef = firstDef.value();
         !ts_node_is_null(currentDef) && ts_node_is_named(currentDef);
      ...
      MATCHV(
          ...
          [&](Node<PipelineContext> &pipeline) {
            mod->pipelines.insert(
                {pipeline->identifier->text, std::move(pipeline)});
          },
          [&](Node<FunctionContext> &function) {
            mod->functions.insert(
                {function->identifier->text, std::move(function)});
          },
          [&](auto &none) {
            errorMessage(
                currentDef,
                std::string(
                    "Grammer was correct, but context created was wrong: ") +
                    typeid(none).name());
          });
    }
    return Some(mod);
  ...
  ParserResult<DocumentContext> parseDocument() {
    tree = ts_parser_parse_string(parser->parser(), nullptr, sourceText.data(),
                                  sourceText.size());
...
ParsedDocument::ParsedDocument(std::filesystem::path path, std::string source,
                               std::shared_ptr<BraneScriptParser> parser)
    : _path(std::move(path)), _source(std::move(source)),
...
ParserResult<DocumentContext> ParsedDocument::getDocumentContext() {
  ...
  ParserAPI ctx{_path,
                _source,
                std::make_shared<TextSource>(
                    TextSource{std::format("file://{}", _path.string())}),
                _parser,
                {},
                {},
                {}};
